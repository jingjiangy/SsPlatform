...
def _build_upload_url(filename: str) -> str:
    # Always store a relative path so the URL stays valid regardless of server
    # IP or hostname changes. The frontend resolves it against VITE_API_BASE at
    # render time, so no database migration is needed when the server moves.
