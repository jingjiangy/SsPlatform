...
async def _device_model_oid_and_label(db, raw_id: str) -> tuple[ObjectId, str]:
    oid = parse_oid(raw_id.strip(), "设备型号ID")
...
async def list_materials(
    ...
    q: dict = {}
    if parent_id:
        q["parent_id"] = parse_oid(parent_id, "父素材ID")
        total = await db["materials"].count_documents(q)
        cur = db["materials"].find(q).sort("_id", -1).skip(page.skip).limit(page.limit)
        items = []
        async for doc in cur:
    ...
    cur = db["materials"].find(_ROOT_PARENT_FILTER).sort("_id", -1).skip(page.skip).limit(page.limit)
    parents = await cur.to_list(length=page.limit)
    if not parents:
...
async def create_material(
    ...
    parent_oid = None
    if body.parent_id:
        parent_oid = parse_oid(body.parent_id, "父素材ID")
    if parent_oid:
