...
_DEFAULT_RETRY_RATIOS: list[float] = [0.8, 0.5, 0.0]
...
class EvalStepDef(BaseModel):
    ...
    name: str
    max_score: float
    retry_ratios: list[float] = Field(default_factory=lambda: list(_DEFAULT_RETRY_RATIOS))
...
def _validate_retry_ratios(v: list[float]) -> list[float]:
    if len(v) != 3:
        raise ValueError("retry_ratios 须包含恰好 3 个值（对应第1/2/3次重试）")
    for r in v:
        if not (0.0 <= r <= 1.0):
            raise ValueError("retry_ratios 每项须在 0~1 之间")
    return [round(r, 4) for r in v]
...
class EvalStepTemplateStep(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=256)
    max_score: float = Field(gt=0, le=1_000_000)
    retry_ratios: list[float] = Field(
        default_factory=lambda: list(_DEFAULT_RETRY_RATIOS),
        description="重试扣分倍率列表，依次对应第1/2/3次重试得分比例，默认 [0.8, 0.5, 0]",
    )

    @field_validator("retry_ratios")
    @classmethod
    def v_retry_ratios(cls, v: list[float]) -> list[float]:
        return _validate_retry_ratios(v)
...
class EvalTaskOut(BaseModel):
    ...
    def from_doc(cls, doc: dict[str, Any]) -> "EvalTaskOut":
        ...
        for s in raw_steps:
            try:
                rr = s.get("retry_ratios")
                out_steps.append(
                    EvalStepDef(
                        id=str(s.get("id", "")),
                        name=str(s.get("name", "")),
                        max_score=float(s.get("max_score", 0)),
                        retry_ratios=rr if isinstance(rr, list) and len(rr) == 3 else list(_DEFAULT_RETRY_RATIOS),
                    )
                )
            except Exception:
                continue
