...
export default function FaultRecordsView() {
  ...
  const canWrite = canWriteFaultRecord();
  const skip = useMemo(() => skipForPage(page, pageSize), [page, pageSize]);

  async function load() {
    const { data } = await http.get("/api/fault-records", { params: { skip, limit: pageSize } });
  ...
  function openAdd() { setMode("add"); setEditingId(null); form.resetFields(); form.setFieldsValue({ status: FAULT_STATUS[0] }); setDlg(true); }
  function openEdit(row: Row) {
    setMode("edit"); setEditingId(String(row._id));
    form.setFieldsValue({ title: row.title, description: row.description || "", status: row.status, robot_id: row.robot_id || "" });
    setDlg(true);
  }

  async function save() {
    const vals = await form.validateFields();
    if (mode === "add") await http.post("/api/fault-records", vals);
    else if (editingId) await http.put(`/api/fault-records/${editingId}`, vals);
    setDlg(false); message.success("已保存"); await load();
  }

  async function remove(row: Row) {
    Modal.confirm({ title: "确定删除该故障记录？", okType: "danger", onOk: async () => {
      await http.delete(`/api/fault-records/${row._id}`); message.success("已删除"); await load();
    }});
  }
  ...
  const columns = [
    ...
    { title: "记录ID", dataIndex: "_id", width: 200 },
    { title: "标题", dataIndex: "title" },
    { title: "描述", dataIndex: "description", ellipsis: true },
    { title: "状态", dataIndex: "status", width: 100 },
    { title: "机器人ID", dataIndex: "robot_id", width: 200, ellipsis: true },
  ...
  return (
    <Card title={<div style={{ display: "flex", justifyContent: "space-between" }}><span>故障记录</span>{canWrite && <Button type="primary" onClick={openAdd}>添加记录</Button>}</div>}>
      <Table rowKey="_id" dataSource={items} columns={columns} pagination={pagination} bordered size="small" scroll={{ x: "max-content" }} />
      <Modal title={mode === "add" ? "添加故障记录" : "修改故障记录"} open={dlg} onOk={save} onCancel={() => setDlg(false)} afterClose={() => form.resetFields()}>
        <Form form={form} labelCol={{ span: 6 }}>
          ...
          <Form.Item label="状态" name="status" rules={[{ required: true }]}><Select options={FAULT_STATUS.map((s) => ({ value: s, label: s }))} /></Form.Item>
          <Form.Item label="机器人ID" name="robot_id"><Input /></Form.Item>
        </Form>
      </Modal>
    </Card>
  );
