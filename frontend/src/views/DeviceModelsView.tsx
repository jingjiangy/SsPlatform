...
export default function DeviceModelsView() {
  ...
  const canWrite = canWriteDeviceModel();
  const skip = useMemo(() => skipForPage(page, pageSize), [page, pageSize]);

  async function load() {
    const { data } = await http.get("/api/device-models", { params: { skip, limit: pageSize } });
  ...
  function openAdd() { setMode("add"); setEditingId(null); form.resetFields(); form.setFieldsValue({ status: DEVICE_MODEL_STATUS[0] }); setDlg(true); }
  ...
  async function save() {
    ...
    if (mode === "add") await http.post("/api/device-models", vals);
    else if (editingId) await http.put(`/api/device-models/${editingId}`, vals);
    setDlg(false); message.success("已保存"); await load();
  }

  async function remove(row: Row) {
    Modal.confirm({ title: "确定删除该设备型号？", okType: "danger", onOk: async () => {
      await http.delete(`/api/device-models/${row._id}`); message.success("已删除"); await load();
    }});
  }
  ...
  const columns = [
    ...
    { title: "型号ID", dataIndex: "_id", width: 200 },
    { title: "型号名称", dataIndex: "name" },
    ...
    ...(canWrite ? [{ title: "操作", width: 160, fixed: "right" as const, render: (_: unknown, row: Row) => (<><Button type="link" onClick={() => openEdit(row)}>修改</Button><Button type="link" danger onClick={() => remove(row)}>删除</Button></>) }] : []),
  ];

  return (
    <Card title={<div style={{ display: "flex", justifyContent: "space-between" }}><span>设备型号</span>{canWrite && <Button type="primary" onClick={openAdd}>添加型号</Button>}</div>}>
      <Table rowKey="_id" dataSource={items} columns={columns} pagination={pagination} bordered size="small" scroll={{ x: "max-content" }} />
      <Modal title={mode === "add" ? "添加设备型号" : "修改设备型号"} open={dlg} onOk={save} onCancel={() => setDlg(false)} afterClose={() => form.resetFields()}>
        <Form form={form} labelCol={{ span: 6 }}>
          <Form.Item label="型号名称" name="name" rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item label="描述" name="description"><Input.TextArea /></Form.Item>
          <Form.Item label="状态" name="status" rules={[{ required: true }]}>
            <Select options={DEVICE_MODEL_STATUS.map((s) => ({ value: s, label: s }))} />
          </Form.Item>
        </Form>
      </Modal>
    </Card>
