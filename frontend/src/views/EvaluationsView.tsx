...
export default function EvaluationsView() {
  ...
  function openEdit(row: Row) {
    ...
    setEditingTemplateName(row.template_name ? String(row.template_name) : null);
  ...
  async function save() {
    ...
    const body: Record<string, unknown> = { description: vals.description, task_type: vals.task_type, status: vals.status, material_name: vals.material_name || undefined };
  ...
  const columns = [
    ...
    { title: "评测模板", width: 200, render: (_: unknown, row: Row) => row.template_id ? <Button type="link" style={{ padding: 0 }} onClick={() => navigate(`/eval-templates?open=${row.template_id}`)}>{String(row.template_name || row.template_id)}</Button> : <span>—</span> },
    { title: "描述", dataIndex: "description", render: (v: string) => <span style={{ whiteSpace: "pre-wrap" }}>{v}</span>, minWidth: 160 },
      ...
      <Modal title="修改评测任务" open={dlg} onOk={save} onCancel={() => setDlg(false)} width={720} afterClose={() => form.resetFields()}>
        <Form form={form} labelCol={{ span: 5 }}>
          ...
          {editingTemplateName != null && (
            <Form.Item label="关联模板"><Input value={editingTemplateName || "（无）"} readOnly /></Form.Item>
          )}
